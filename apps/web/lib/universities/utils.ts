...
export function calculateAIMatch(university: University, profile: UserProfile): AIMatchResult {
  let matchScore = 50
  ...
  if (profile.ielts >= university.ieltsRequirement) {
    reasons.push(`Your IELTS (${profile.ielts}) meets standards with margin`)
  }
  if (profile.budget >= university.tuition.max) {
