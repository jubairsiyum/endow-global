    ...
    <div className="space-y-6">
      ...
      <AdminTable>
        <div className="overflow-x-auto">
          <div className="grid min-w-[900px] grid-cols-6 border-b border-gray-100 bg-gray-50 px-6 py-4 text-sm font-semibold text-gray-600 dark:border-gray-800 dark:bg-[#222530] dark:text-gray-300">
            <div>Student</div>
          ...
          {isLoading ? (
            <div className="py-10">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="grid min-w-[900px] grid-cols-6 items-center border-b border-gray-100 px-6 py-5 dark:border-gray-800">
                  {Array.from({ length: 6 }).map((_, j) => (
                    <div key={j} className="h-4 w-24 animate-pulse rounded bg-gray-200 dark:bg-[#222530]" />
                  ))}
                </div>
              ))}
            </div>
            ...
            documents.map(doc => (
              <div key={doc.id} className="grid min-w-[900px] grid-cols-6 items-center border-b border-gray-100 px-6 py-5 transition-all hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-[#1a1d25]">
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">{doc.studentName}</p>
                  <p className="text-xs text-gray-400">{doc.studentEmail}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{doc.university}</p>
      ...
      {/* PREVIEW MODAL */}
      {previewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm" onClick={() => setPreviewUrl(null)}>
