...
export default function DepartmentsPage() {
 ...
 <div className="text-sm text-gray-600">{d.university?.name || '—'}</div>
 <div className="truncate text-sm text-gray-500 max-w-[200px]">{d.description || '—'}</div>
 <div className="flex items-center gap-2">
 <button onClick={() => openEdit(d)} className="rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition-all hover:bg-gray-200">
 <Pencil size={14} />
 </button>
 {deleteConfirm === d.id ? (
 ...
 <button onClick={() => setDeleteConfirm(d.id)} className="rounded-xl bg-red-200 px-3 py-2 text-sm font-medium text-red-600 transition-all hover:bg-red-200">
 <Trash2 size={14} />
 </button>
 )}
 </div>
 ...
 <h2 className="text-xl font-bold text-gray-900">{editingId ? 'Edit Department' : 'Add Department'}</h2>
 ...
 <select required value={form.universityId} onChange={e => setForm({ ...form, universityId: e.target.value })} className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-primary">
 ...
 <input value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} placeholder="e.g. CS" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-primary" />
 </div>
 <div>
