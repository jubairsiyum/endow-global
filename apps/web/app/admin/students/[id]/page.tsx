...
export default function StudentDetailPage() {
  ...
  if (isLoading) {
    return <div className="py-20 text-center">Loading student details...</div>;
  }
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="col-span-2 space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-[#1a1d25]">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Academic Profile</h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">Highest Education</p>
                <p className="font-medium text-gray-900 dark:text-white">{profile?.highestEducation || "N/A"}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-gray-900 dark:text-white">{profile?.ieltsScore || "N/A"}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">TOEFL Score</p>
                <p className="font-medium text-gray-900 dark:text-white">{profile?.toeflScore || "N/A"}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Target Countries</p>
                <p className="font-medium text-gray-900 dark:text-white">
                  {Array.isArray(profile?.targetCountries) ? profile.targetCountries.join(", ") : "None"}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                {profile.applications.map((app: any) => (
                  <div key={app.id} className="rounded-lg border p-4 dark:border-gray-700">
                    ...
                    <p className="mt-2 text-sm">Status: <span className="font-medium">{app.status}</span></p>
                  </div>
                ))}
              </div>
            ) : (
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-[#1a1d25]">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Counselor Assignment</h2>
            <div className="space-y-4">
              <select
                ...
                onChange={(e) => assignMutation.mutate({ studentId: id, counselorId: e.target.value || null })}
