...
export default function SAUniversitiesPage() {
  ...
  function startEdit(uni: any) {
    ...
    setForm({
      ...
      rankingsJson: Array.isArray(uni.rankings) ? (uni.rankings as any[]).map((r: any) => `${r.body || ''}: ${r.position || ''} (${r.year || ''}) â€” ${r.category || ''}`).join('\n') : (typeof uni.rankings === 'string' ? uni.rankings : ''),
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const rankingsData = form.rankingsJson
      ? form.rankingsJson.split('\n').map((line) => {
          const match = line.match(/^(.+?):\s*(.+?)\s*\((\d{4})\)(?:\s*â€”\s*(.+))?$/)
  ...
  const inputStyle = { background: '#f8fafc', borderColor: '#e5e7eb', color: '#111827' } as const
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-4">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold tracking-tight" style={{ color: '#111827', fontFamily: "'Space Grotesk', sans-serif" }}>Universities</h1>
          <p className="mt-0.5 text-[13px]" style={{ color: '#6b7280' }}>Manage partner university catalog</p>
        </div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border p-6" style={{ background: '#ffffff', borderColor: '#e5e7eb' }}>
          <h2 className="mb-5 text-[16px] font-semibold" style={{ color: '#111827', fontFamily: "'Space Grotesk', sans-serif" }}>{editingId ? 'Edit University' : 'Add New University'}</h2>
          <form onSubmit={handleSubmit} className="space-y-5">

            {/* â”€â”€ Basic Information â”€â”€ */}
            <div>
              <div className="mb-3 flex items-center gap-2 pb-2 border-b" style={{ borderColor: '#e5e7eb' }}><Building2 size={14} style={{ color: '#4FD1A5' }} /><span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#4FD1A5' }}>Basic Information</span></div>
            ...
            {/* â”€â”€ Brand Assets â”€â”€ */}
            <div>
              <div className="mb-3 flex items-center gap-2 pb-2 border-b" style={{ borderColor: '#e5e7eb' }}><Image size={14} style={{ color: '#E8A33D' }} /><span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#E8A33D' }}>Brand Assets</span></div>
            ...
            {/* â”€â”€ Rankings & Stats â”€â”€ */}
            <div>
              <div className="mb-3 flex items-center gap-2 pb-2 border-b" style={{ borderColor: '#e5e7eb' }}><Award size={14} style={{ color: '#a78bfa' }} /><span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#a78bfa' }}>Rankings & Statistics</span></div>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="mt-3 grid grid-cols-1 gap-3">
                <label className="flex flex-col gap-1">
                  <span className="text-[11px] font-medium" style={{ color: '#6b7280' }}>Detailed Rankings (one per line: "Body: Position (Year) â€” Category")</span>
                  <textarea value={form.rankingsJson} onChange={(e) => setField('rankingsJson', e.target.value)} rows={4}
                    placeholder={`QS World Rankings: 32 (2024) â€” Global\nTimes Higher Education: 25 (2024) â€” World\nFinancial Times: 15 (2025) â€” UK Business Schools`}
                    className="rounded-md border px-3 py-1.5 text-[13px] outline-none resize-y" style={{ ...inputStyle, minHeight: 80 }} />
                </label>
              </div>
            </div>

            {/* â”€â”€ Accreditation & Reputation â”€â”€ */}
            <div>
              <div className="mb-3 flex items-center gap-2 pb-2 border-b" style={{ borderColor: '#e5e7eb' }}><Star size={14} style={{ color: '#E8A33D' }} /><span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#E8A33D' }}>Accreditation & Reputation</span></div>
              <label className="flex flex-col gap-1">
                <span className="text-[11px] font-medium" style={{ color: '#6b7280' }}>Accreditation Details</span>
            ...
            <div>
              <div className="mb-3 flex items-center gap-2 pb-2 border-b" style={{ borderColor: '#e5e7eb' }}><Eye size={14} style={{ color: '#6b7280' }} /><span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#6b7280' }}>Status & Visibility</span></div>
              <div className="flex flex-wrap gap-x-6 gap-y-2">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <Star size={13} style={{ color: form.featured ? '#E8A33D' : '#6b7280' }} />
                  <span className="text-[12px]" style={{ color: form.featured ? '#111827' : '#6b7280' }}>Featured University</span>
                </label>
              </div>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr style={{ background: '#ffffff' }}>{['Logo', 'Name', 'Country', 'City', 'Ranking', 'Students', 'Accredited', 'Status', 'Actions'].map((h) => <th key={h} className="px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#6b7280', fontFamily: "'JetBrains Mono', monospace" }}>{h}</th>)}</tr></thead>
              <tbody className="[&_tr]:border-t [&_tr]:border-[#e5e7eb]/50">
                {(uniList ?? []).map((uni: any) => (
                  <tr key={uni.id} className="transition-colors hover:bg-[#E8A33D]/[0.04]">
                    <td className="px-3 py-3">
                      <div className="h-8 w-8 relative rounded-lg overflow-hidden border flex items-center justify-center shrink-0" style={{ borderColor: '#e5e7eb', background: '#f8fafc' }}>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-[13px] font-medium block truncate max-w-[200px]" style={{ color: '#111827' }}>{uni.name}</span>
                        {uni.featured && <Star size={11} style={{ color: '#E8A33D' }} />}
                      </div>
                    </td>
                    <td className="px-3 py-3 text-[12px]" style={{ color: '#6b7280' }}><div className="flex items-center gap-1"><Globe size={11} />{uni.country}</div></td>
                    <td className="px-3 py-3 text-[12px]" style={{ color: '#6b7280' }}><div className="flex items-center gap-1"><MapPin size={11} />{uni.city}</div></td>
                    <td className="px-3 py-3 text-[12px]" style={{ color: '#111827', fontFamily: "'JetBrains Mono', monospace" }}>{uni.ranking ? `#${uni.ranking}` : 'â€”'}</td>
                    <td className="px-3 py-3 text-[12px]" style={{ color: '#111827' }}>{uni.totalStudents?.toLocaleString() || 'â€”'}</td>
                    <td className="px-3 py-3">{uni.accreditation ? <SABadge variant="success">Accredited</SABadge> : <span className="text-[11px]" style={{ color: '#6b7280' }}>â€”</span>}</td>
                ...
                {(!uniList || uniList.length === 0) && (<tr><td colSpan={9} className="py-20 text-center"><Building2 size={28} style={{ color: '#6b7280', margin: '0 auto 8px' }} /><p className="text-[13px]" style={{ color: '#6b7280' }}>{search ? 'No universities match your search' : 'No universities yet. Add your first one.'}</p></td></tr>)}
              </tbody>
            </table>
          </div>
        )}
